...
CustomTextStyle googleFontsTextStyle({
  ...
  textStyle = textStyle.copyWithCustom(
    color: color,
...
Future<ByteData?> getFontByte(GoogleFontsDescriptor descriptor) async {
  final familyWithVariantString = descriptor.familyWithVariant.toString();
  ...
  try {
    ...
    if (await byteData != null) return await byteData;

    // Check if this font can be loaded from the device file system.
    byteData = file_io.loadFontFromDeviceFileSystem(name: familyWithVariantString, fileHash: fileHash);
